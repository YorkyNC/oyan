...
class ChevronChip extends StatelessWidget {
  const ChevronChip({super.key, required this.label, required this.onTap});
  ...
  Widget build(BuildContext context) {
    return ActionChip(
      ...
      label: Row(
        children: [
          Text(label, style: context.typography.typography1SemiBold.copyWith(color: context.colors.gray700)),
          SvgPicture.asset(
            context.assetImages.chevronRight,
            colorFilter: ColorFilter.mode(context.colors.gray700, BlendMode.srcIn),
          ),
        ],
      ),
