...
import 'package:oyan/src/features/attachment/data/models/add_attachment_response.dart';
...
class AttachmentBloc extends BaseBloc<AttachmentEvent, AttachmentState> {
  AttachmentBloc(this.addAttachmentUseCase, this.getAttachmentUseCase) : super(const AttachmentState.loading());
  ...
  Future<void> onEventHandler(AttachmentEvent event, Emitter emit) async {
    await event.when(
      ...
      addAttachment: (attachment) => _addAttachment(event as _AddAttachment, emit as Emitter<AttachmentState>),
      getAttachment: (request) => _getAttachment(event as _GetAttachment, emit as Emitter<AttachmentState>),
    );
  }
  ...
  Future<void> _addAttachment(_AddAttachment event, Emitter emit) async {
    final Result<AddAttachmentResponse, DomainException> result = await addAttachmentUseCase.call(event.request);

    emit(const AttachmentState.loading());

    if (result.isSuccessful) {
      _viewModel = _viewModel.copyWith(attachment: AttachmentEntity(id: result.data!.id));
    }
  ...
  Future<void> _getAttachment(_GetAttachment event, Emitter emit) async {
    emit(const _Loading());
    final Result<AttachmentEntity, DomainException> result = await getAttachmentUseCase.call(event.request);

    final data = result.data;

    if (data != null) {
      _viewModel = _viewModel.copyWith(attachment: data);
      return emit(_Loaded(viewModel: _viewModel.copyWith(attachment: data)));
    }
