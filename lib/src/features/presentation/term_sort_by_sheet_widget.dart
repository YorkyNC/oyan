...
class TermSortBySheetWidget extends StatefulWidget {
  ...
  const TermSortBySheetWidget({super.key, required this.title, required this.onTermSelected, this.currentTerm});

  @override
  _SortBySheetState createState() => _SortBySheetState();
}

class _SortBySheetState extends State<TermSortBySheetWidget> {
  late int? selectedTerm = widget.currentTerm;
  BoxDecoration _getBoxDecoration(int? term) {
    return BoxDecoration(
      color: selectedTerm == term ? const Color(0xFFDBF0F1) : Colors.transparent,
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(widget.title, style: const FigmaTextStyles().typography4Medium.copyWith(color: context.colors.black)),
  ...
  Widget _buildOption({required int? term, required String label}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            Text(label, style: const FigmaTextStyles().typography3Medium.copyWith(color: context.colors.black)),
          ],
        ),
      ),
    );
