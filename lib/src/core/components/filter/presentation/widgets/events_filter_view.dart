...
import 'package:oyan/src/core/components/events/domain/extensions/event_type_extension.dart';
...
class _EventsFilterViewState extends State<EventsFilterView> {
  ...
  void _onDateSelected(DateFilter dateFilter) {
    setState(() {
      _currentFilter = _currentFilter.copyWith(date: dateFilter);
    });
  ...
  void _clear() {
    setState(() {
      _currentFilter = EventsFilter(date: DateFilter.day(startDate: DateTime.now()));
    });
    widget.onFilterSelected?.call(_currentFilter);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Text(context.loc.byDate, style: context.typography.typography1SemiBold.copyWith(color: context.colors.black)),
        DateFilterView(dateFilter: _currentFilter.date, onSelected: _onDateSelected),
        const SizedBox(height: 10),
        Text(context.loc.byType, style: context.typography.typography1SemiBold.copyWith(color: context.colors.black)),
        ...
        ElevatedButton(
          ...
          child: Row(
            ...
            children: [
              Text(context.loc.showResults),
              const SizedBox(width: 8),
              (count == null)
                  ? const Padding(
                    padding: EdgeInsets.only(left: 1),
                    child: SizedBox(width: 12, height: 12, child: CircularProgressIndicator(strokeWidth: 1.2)),
                  )
                  : Text("($count)"),
            ],
