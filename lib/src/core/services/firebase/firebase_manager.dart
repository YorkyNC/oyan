...
import 'package:oyan/firebase_options.dart';
import 'package:oyan/src/core/api/client/endpoints.dart';
...
class FirebaseManager {
  ...
  static Future<bool> _patchDevice(String currentToken, String clientId, bool includeToken) async {
    try {
      ...
      final response = await dio.patch('${EndPoints.baseUrl}/v1/user-device/$clientId', data: data);
  ...
  static Future<void> _registerNewDevice(String token) async {
    try {
      ...
      final response = await dio.post(
        '/v1/user-device',
        data: {'userId': userId, 'token': token, 'deviceInfo': deviceData},
      );
  ...
  static Future<void> deactivateDeviceToken() async {
    try {
      ...
      final dio = getIt<Dio>();
      final response = await dio.patch('/v1/user-device/$clientId', data: {'userId': null, 'deviceInfo': deviceData});
  ...
  static Future<DeviceInfo> _getDeviceInfo() async {
    ...
    return DeviceInfo(platform: platform, model: model, deviceId: deviceId);
  }
}
...
class DeviceInfo {
  ...
  DeviceInfo({required this.platform, required this.model, required this.deviceId});
}
