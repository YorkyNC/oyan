...
class ConnectionStatusNotifier extends ChangeNotifier {
  ...
  Future<void> checkConnectivity() async {
    ...
    try {
      ...
      _checkState = hasConnection ? ConnectionCheckState.success : ConnectionCheckState.failure;
      ...
      _stateResetTimer = Timer(const Duration(seconds: 2), () {
        if (!_hasConnection &&
            (_checkState == ConnectionCheckState.success || _checkState == ConnectionCheckState.failure)) {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: const FigmaTextStyles().typography4Medium.copyWith(color: context.colors.black),
                  ),
                  const SizedBox(height: 8),
                  Consumer<ConnectionStatusNotifier>(
                    builder: (context, connectionStatus, _) {
                      final message = switch (connectionStatus.checkState) {
                        ConnectionCheckState.checking => context.loc.checkingConnection,
                        ConnectionCheckState.success => context.loc.connectionRestored,
                        ConnectionCheckState.failure => context.loc.noConnection,
                  ...
                  Consumer<ConnectionStatusNotifier>(
                    builder: (context, connectionStatus, _) {
                      final isChecking = connectionStatus.checkState == ConnectionCheckState.checking;

                      return InkWell(
                        splashColor: Colors.transparent,
                        highlightColor: Colors.transparent,
                        onTap: isChecking ? null : () => connectionStatus.checkConnectivity(),
                        child: Container(
                                  ...
                                  isChecking
                                      ? SizedBox(
                                        ...
                                        child: CircularProgressIndicator(
                                          strokeWidth: 2,
                                          valueColor: AlwaysStoppedAnimation<Color>(context.colors.blue500),
                                        ),
                                      )
                                      : Text(
