...
class RoleNotifier extends ChangeNotifier {
  ...
  void _handleTeacherRole(dynamic role, String? teacherId) {
    var existingRole = _roleMap['teacher']!.firstWhere(
      ...
      orElse: () {
        var newRole = RoleInfo(roleTitle: 'teacher', teacherId: teacherId, courseIds: []);
  ...
  void _handleClassroomTeacherRole(dynamic role, String? teacherId, String classId) {
    if (!_roleMap['classroomTeacher']!.any((r) => r.classId == classId)) {
      _roleMap['classroomTeacher']!.add(
        RoleInfo(roleTitle: 'classroomTeacher', teacherId: teacherId, classId: classId),
      );
    }
  }
  ...
  void _handleParentRole(dynamic role, String? teacherId) {
    if (role.childId != null) {
      var existingRole = _roleMap['parent']!.firstWhere(
        (r) => r.teacherId == teacherId,
        orElse: () {
          var newRole = RoleInfo(roleTitle: 'parent', teacherId: teacherId, childIds: []);
  ...
  void setRole(String role, String roleId, {String? classId}) {
    print('RoleNotifier setRole - Current: $_currentRole, $_currentRoleId, $_currentClassId'); // Debug
    ...
    bool needsUpdate =
        _currentRole != role || _currentRoleId != roleId || (classId != null && _currentClassId != classId);
    ...
    if (needsUpdate) {
      ...
      print('RoleNotifier setRole - Updated state: $_currentRole, $_currentRoleId, $_currentClassId'); // Debug
      notifyListeners();
    }
  }

  // Helper methods
  bool hasRole(String roleTitle) => _roleMap.containsKey(roleTitle) && _roleMap[roleTitle]!.isNotEmpty;
  ...
  RoleInfo? getCurrentRoleInfo() {
    ...
    return _roleMap[_currentRole]!.firstWhere((roleInfo) {
      switch (_currentRole) {
        case 'classroomTeacher':
